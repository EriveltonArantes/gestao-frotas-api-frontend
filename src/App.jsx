...
const apiBaseUrl = "https://gestao-frotas-api.onrender.com";
    ...
    <div className="login-screen">
      <div className="login-institucional">
        ...
        <ul className="login-features">
          ...
          <li>✓ Controle completo de Motorista</li>
          <li>✓ Controle completo de Abastecimento</li>
          <li>✓ Controle completo de Manutencao Frota</li>
        </ul>
      </div>
...
function PainelVeiculo({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const itensFiltrados = itens.filter(item => !busca || ["placa", "modelo", "marca", "ano", "km", "tipo", "combustivel", "setor", "ativo", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  ...
  const editar = (item) => {
    const f = {};
    ["placa", "modelo", "marca", "ano", "km", "tipo", "combustivel", "setor", "ativo"].forEach(k => { f[k] = item[k] ?? ""; });
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["placa"] ?? "Veiculo")}</div>
            ...
            <div className="item-field"><b>km</b><span>{item["km"] ?? "—"}</span></div>
            <div className="item-field"><b>tipo</b><span>{item["tipo"] ?? "—"}</span></div>
            <div className="item-field"><b>combustivel</b><span>{item["combustivel"] ?? "—"}</span></div>
      ...
      <input type="text" value={form["combustivel"] ?? ""} onChange={e => setForm({...form, combustivel: e.target.value})} />
...
function PainelMotorista({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["nome", "cnh", "categoria", "telefone", "setor", "ativo", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/motoristas", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/motoristas/" + editId : apiBaseUrl + "/api/motoristas";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "cnh", "categoria", "telefone", "setor", "ativo"].forEach(k => { f[k] = item[k] ?? ""; });
    [].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/motoristas/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["nome"] ?? "Motorista")}</div>
            
            <div className="item-meta-grid">
            <div className="item-field"><b>cnh</b><span>{item["cnh"] ?? "—"}</span></div>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Motorista</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">nome</label>
      <input type="text" value={form["nome"] ?? ""} onChange={e => setForm({...form, nome: e.target.value})} />
      <label className="field-label">cnh</label>
      <input type="text" value={form["cnh"] ?? ""} onChange={e => setForm({...form, cnh: e.target.value})} />
...
function PainelAbastecimento({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  const [busca, setBusca] = React.useState("");
  const [pagina, setPagina] = React.useState(1);
  const [motoristaList, setMotoristaList] = React.useState([]);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["veiculoId", "motoristaId", "dataHora", "litros", "preco", "posto", "km", "tipoCombustivel", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/abastecimentos", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  React.useEffect(() => {
    carregar();
    fetch(apiBaseUrl + "/api/motoristas", { headers: authHeaders(token) }).then(r => r.json()).then(setMotoristaList).catch(() => {});
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/abastecimentos/" + editId : apiBaseUrl + "/api/abastecimentos";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["veiculoId", "motoristaId", "dataHora", "litros", "preco", "posto", "km", "tipoCombustivel"].forEach(k => { f[k] = item[k] ?? ""; });
    ["veiculo", "motorista"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/abastecimentos/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["posto"] ?? "Abastecimento")}</div>
            
            <div className="item-meta-grid">
            <div className="item-field"><b>veiculoId</b><span>{item["veiculoId"] ?? "—"}</span></div>
            <div className="item-field"><b>motoristaId</b><span>{item["motoristaId"] ?? "—"}</span></div>
            ...
            <div className="item-field"><b>litros</b><span>{item["litros"] != null ? Number(item["litros"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            <div className="item-field"><b>preco</b><span>{item["preco"] != null ? Number(item["preco"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            <div className="item-field"><b>km</b><span>{item["km"] ?? "—"}</span></div>
            <div className="item-field"><b>tipoCombustivel</b><span>{item["tipoCombustivel"] ?? "—"}</span></div>
            <div className="item-field"><b>veiculo</b><span>{item.veiculoId ? ((veiculoList.find(o => o.id === item.veiculoId) || {}).placa ?? ("#" + item.veiculoId)) : "—"}</span></div>
            <div className="item-field"><b>motorista</b><span>{item.motoristaId ? ((motoristaList.find(o => o.id === item.motoristaId) || {}).nome ?? ("#" + item.motoristaId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
              <button className="btn btn-ghost btn-sm" onClick={() => gerarPix(item.preco)}>Cobrar via Pix</button>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Abastecimento</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
      ...
      <label className="field-label">motoristaId</label>
      <input type="number" value={form["motoristaId"] ?? ""} onChange={e => setForm({...form, motoristaId: e.target.value})} />
      ...
      <label className="field-label">preco</label>
      ...
      <input type="text" value={form["posto"] ?? ""} onChange={e => setForm({...form, posto: e.target.value})} />
      <label className="field-label">km</label>
      <input type="number" value={form["km"] ?? ""} onChange={e => setForm({...form, km: e.target.value})} />
      <label className="field-label">tipoCombustivel</label>
      <input type="text" value={form["tipoCombustivel"] ?? ""} onChange={e => setForm({...form, tipoCombustivel: e.target.value})} />
      <label className="field-label">veiculo</label>
      <select value={form["veiculo"] ?? ""} onChange={e => setForm({...form, veiculo: e.target.value})}>
        <option value="">Selecione...</option>
        {(veiculoList || []).map(o => (<option key={o.id} value={o.id}>{o.placa ?? ("#" + o.id)}</option>))}
      </select>
      <label className="field-label">motorista</label>
      <select value={form["motorista"] ?? ""} onChange={e => setForm({...form, motorista: e.target.value})}>
        <option value="">Selecione...</option>
        {(motoristaList || []).map(o => (<option key={o.id} value={o.id}>{o.nome ?? ("#" + o.id)}</option>))}
      </select>
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
                <button type="button" className="btn btn-ghost" onClick={() => setModalAberto(false)} disabled={salvando}>Cancelar</button>
...
function PainelManutencaoFrota({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["veiculoId", "tipo", "descricao", "custo", "km", "data", "oficina", "status", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/manutencaofrotas", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/manutencaofrotas/" + editId : apiBaseUrl + "/api/manutencaofrotas";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["veiculoId", "tipo", "descricao", "custo", "km", "data", "oficina", "status"].forEach(k => { f[k] = item[k] ?? ""; });
    ["veiculo"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/manutencaofrotas/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
            ...
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["tipo"] ?? "ManutencaoFrota")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/manutencaofrotas/" + item.id, {
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["tipo"] ?? "ManutencaoFrota")}</div>
            ...
            <div className="item-field"><b>custo</b><span>{item["custo"] != null ? Number(item["custo"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            <div className="item-field"><b>km</b><span>{item["km"] ?? "—"}</span></div>
            ...
            <div className="item-field"><b>oficina</b><span>{item["oficina"] ?? "—"}</span></div>
            <div className="item-field"><b>veiculo</b><span>{item.veiculoId ? ((veiculoList.find(o => o.id === item.veiculoId) || {}).placa ?? ("#" + item.veiculoId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} ManutencaoFrota</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">veiculoId</label>
      <input type="number" value={form["veiculoId"] ?? ""} onChange={e => setForm({...form, veiculoId: e.target.value})} />
      ...
      <input type="text" value={form["oficina"] ?? ""} onChange={e => setForm({...form, oficina: e.target.value})} />
...
export default function App() {
  ...
  const [aba, setAba] = React.useState("Veiculo");
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="sidebar-logo">Gestao Frotas<span className="dot">.</span></div>
        <button className={"nav-btn" + (aba === "Veiculo" ? " active" : "")} onClick={() => setAba("Veiculo")}><span className="nav-ico">🚗</span>Veiculo</button>
        <button className={"nav-btn" + (aba === "Motorista" ? " active" : "")} onClick={() => setAba("Motorista")}><span className="nav-ico">🗂️</span>Motorista</button>
        <button className={"nav-btn" + (aba === "Abastecimento" ? " active" : "")} onClick={() => setAba("Abastecimento")}><span className="nav-ico">🗂️</span>Abastecimento</button>
        <button className={"nav-btn" + (aba === "ManutencaoFrota" ? " active" : "")} onClick={() => setAba("ManutencaoFrota")}><span className="nav-ico">🗂️</span>ManutencaoFrota</button>
      ...
      {aba === "Motorista" && <PainelMotorista token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Abastecimento" && <PainelAbastecimento token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "ManutencaoFrota" && <PainelManutencaoFrota token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      </main>
